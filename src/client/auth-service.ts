...
import { MediaListStatus, UserData } from "./user-info-types";
...
export const buildAuthUrl = (csrfToken: string): string => {
  ...
  const encodedRedirectedUri = encodeURIComponent(redirectUri);

  return `https://anilist.co/api/v2/oauth/authorize?clinet_id=${clientId}&scope=${scope}&response_type=code&redirect_uri=${encodedRedirectedUri}&state=${state}`;
};
...
export const useAnimeList = (username: string, status: MediaListStatus) => {
